...
import requests
# import os # Non più necessario per il token
import logging
...
# --- TOKEN DEL BOT TELEGRAM HARDCODED ---
# Come richiesto, il token è inserito direttamente nel codice.
# ATTENZIONE: Questa pratica non è sicura per la produzione o per repository pubblici.
...
def get_joke():
    ...
    try:
        ...
        # Assicurati che 'setup' e 'punchline' esistano nella risposta
        if 'setup' in joke_data and 'punchline' in joke_data:
            return f"{joke_data['setup']}\n\n...\n\n{joke_data['punchline']}"
        else:
            logger.error(f"Formato della risposta dell'API inaspettato: {joke_data}")
    ...
    except (KeyError, TypeError, ValueError) as e: # Aggiunto ValueError per json decode error
...
def main():
    """Avvia il bot."""
    if not TELEGRAM_BOT_TOKEN: # Controllo comunque utile se si dovesse cambiare idea
    ...
    dp.add_handler(CommandHandler("joke", barzelletta))
